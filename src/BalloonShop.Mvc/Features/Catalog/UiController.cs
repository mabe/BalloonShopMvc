...
namespace BalloonShop.Mvc.Features.Catalog
{
	...
	public class UiController : Controller
	{
		...
		{
			var d = _session.QueryOver<Department>().Where(x => x.Name == CatalogUrlHelper.DeFilter(department)).SingleOrDefault();
		...
		{
			var c = _session.QueryOver<Category>().Where(x => x.Name == CatalogUrlHelper.DeFilter(category)).SingleOrDefault();
